...
-- ==============================================================================
-- MIGRA��ES E CORRE��ES (M1, M3, Fixes)
-- ==============================================================================

-- SQL/M1_Alunos_CEP.sql
-- Adiciona colunas de endereço à tabela de alunos
...
-- Previne a duplicidade de matrículas ativas e de certificados, e classifica renovações.
...
BEGIN
    ...
    -- 2. Verifica se já possui certificado válido
    SELECT EXISTS (
    ...
    IF v_cert_valido THEN
        RETURN jsonb_build_object('autorizado', false, 'motivo', 'Aluno já possui um certificado válido para este curso.');
    END IF;

    -- 3. Verifica renovação/reciclagem
    SELECT EXISTS (
        SELECT 1 FROM public.certificados 
...
-- ============================================================
-- EduOS — Triggers SQL de apoio aos 4 fixes críticos
-- Aplique no Supabase SQL Editor (Dashboard → SQL Editor)
-- ============================================================

-- ─── FIX 1: Manter turmas.ocupadas sincronizado automaticamente ──────────────
-- Trigger que incrementa/decrementa turmas.ocupadas ao inserir/atualizar matriculas.
-- Funciona como fallback ao ajuste feito no JS (matriculas.js → adjustOcupadas).
...
BEGIN
  ...
  -- UPDATE: troca de turma ou mudança de status (cancelado)
  ELSIF TG_OP = 'UPDATE' THEN

    -- Cancelou a matrícula que tinha turma → decrementa
...
-- ─── FIX 2: Status de certificados calculado por coluna gerada ───────────────
...
-- ─── FIX 3: Marcar pagamentos em atraso automaticamente ──────────────────────
-- Stored procedure que pode ser chamada via cron (pg_cron) ou manualmente.
-- O JS (financeiro.js → autoMarkAtrasados) já resolve isso no frontend,
-- mas esta função garante consistência mesmo sem abrir o app.
...
-- Para agendar via pg_cron (executar 1x por dia à meia-noite):
